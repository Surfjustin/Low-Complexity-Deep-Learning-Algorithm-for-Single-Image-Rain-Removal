...
class MySCN(nn.Module):
    def __init__(self):
        ...
        self.W1 = nn.Conv2d(32, 64, 3, 1, 1, bias=False)
        self.S = nn.Conv2d(64, 64, 3, 1, 1, groups=1, bias=False)
        self.W2 = nn.Conv2d(64, 32, 3, 1, 1, bias=False)
        self.RW1 = CALayer(64,16)
        self.RW2 = CALayer(64,16)
        
        self.RWC1 = CALayera(64,16)
        self.RWC2 = CALayera(64,16)
    ...
    def forward(self, input):
        ...
        for _ in range(8):
            ...
            x = self.RWC2(self.shlu( self.RWC1(tmp)[0] - self.T))[0]
            x = self.S(x)
            tmp = x + z

        c = self.RWC2(self.shlu( self.RWC1( x+z )[0] - self.T))[0]
        ####################  rain density estimation  ########################################
        #rde = self.RWC1( x+z )[1].data[0,:,0,0].cpu().numpy().astype(np.float32)
        #print(np.mean(rde))
        #f = open('rde.txt','a')
        #f.write('{} \n'.format(np.mean(rde)))
        #f.close()
...
class RlcscNet(nn.Module):
    def __init__(self):
        ...
        self.advanced = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, bias=False)
...
## channel attention layer for AC(act->conv)
class CALayera(nn.Module):
    def __init__(self,channel,reduction=16):
        super(CALayera,self).__init__()
        self.conv_atten = nn.Sequential(
...
# 定義一個簡單的CNN模型，這裡以ResNet18為例
model = RlcscNet().cuda()

# 假設輸入張量的大小為 (batch_size, channels, height, width)
input_tensor = torch.randn(1, 3, 512, 512)
input_tensor = input_tensor.cuda()
# 利用 thop 的 profile 函數計算 FLOPs
flops, params = profile(model, inputs=(input_tensor,))

# 使用 clever_format 函數將 FLOPs 和參數格式化為人類可讀的形式
